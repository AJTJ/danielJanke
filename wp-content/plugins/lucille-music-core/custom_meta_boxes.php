...
function LUCILLE_SWP_custom_bg_meta() {
	...
	if ('template-discography.php' == $page_template) {
		add_meta_box(
			...
			esc_html__("Discography Settings - Lucille Theme", 'lucille-music-core'), 
			"LUCILLE_SWP_discography_settings_cbk", 
			"page");		
	}

	/*add custom meta to contact page template*/
	if ('template-contact.php' == $page_template) {
		add_meta_box(
			"js_swp_contact_settings_meta", 
			esc_html__("Contact Page Settings", 'lucille-music-core'), 
			"LUCILLE_SWP_contact_settings_cbk", 
			"page");		
	}
	...
	*/
	...
	foreach($artist_select_support as $post_type) {
		add_meta_box( 
			...
			esc_html__("Artist Selection", 'lucille-music-core'),
			'LUCILLE_SWP_artist_select_meta_callback',  
			$post_type,
			'advanced',	/*context*/
			'high'	/*priority*/
		);
...
/*
	Contact settings meta
*/
function LUCILLE_SWP_contact_settings_cbk($post) {
	$stored_meta = get_post_meta($post->ID);
	
	$string1 = $string2 = '';
	/*SEND US A MESSAGE*/
	if (isset($stored_meta['lc_swp_contact_string1'])) {
		$string1 = $stored_meta['lc_swp_contact_string1'][0];
	}

	/*WHERE TO FIND US*/
	if (isset($stored_meta['lc_swp_contact_string2'])) {
		$string2 = $stored_meta['lc_swp_contact_string2'][0];
	}
?>

	<div class="heading_meta_option">
		<span class="lc_swp_before_option"><?php echo esc_html__("String to replace [SEND ME A MESSAGE]:", "lucille-music-core"); ?></span>
		<input type="text" name="lc_swp_contact_string1" id="lc_swp_contact_string1" size="150" value="<?php echo esc_attr($string1); ?>" />
	</div>
	<div class="heading_meta_option">
		<span class="lc_swp_before_option"><?php echo esc_html__("String to replace [WHERE TO FIND US]:", "lucille-music-core"); ?></span>
		<input type="text" name="lc_swp_contact_string2" id="lc_swp_contact_string2" size="150" value="<?php echo esc_attr($string2); ?>" />		
	</div>
<?php
}
...
/*
	Artist Selection
*/
function LUCILLE_SWP_artist_select_meta_callback($post) {
	$stored_meta = get_post_meta($post->ID);
...
?>
	<div class="heading_meta_option">
		<span class="lc_swp_before_option"><?php echo esc_html__("Select one or more artists:", "lucille-music-core"); ?></span>
		...
		<?php
			foreach($artist_posts as $single_artist) {
				if (in_array($single_artist->ID, $artist_selection)) {
					?>
					<option value="<?php echo $single_artist->ID; ?>" selected="selected"> <?php echo $single_artist->post_title; ?> </option>
					<?php
				} else {
					?>
					<option value="<?php echo $single_artist->ID; ?>"> <?php echo $single_artist->post_title; ?> </option>
					<?php
				}
			}
		?>
		...
		<p class="description for_artist_selection show_on_right">
			<?php echo esc_html__('Select artists for this event/video/album. Hold down the Ctrl (windows) or Command (Mac) button to select multiple options.', 'lucille-music-core'); ?>
		</p>
	</div>
<?php
}
...
function LUCILLE_SWP_contact_settings_meta_save($post_id) {
	$is_autosave = wp_is_post_autosave( $post_id );
    $is_revision = wp_is_post_revision( $post_id );
	...
	if(isset($_POST['lc_swp_contact_string1'])) {
		update_post_meta($post_id, 'lc_swp_contact_string1', trim($_POST['lc_swp_contact_string1'])) ;
	} else {
		update_post_meta($post_id, 'lc_swp_contact_string1', '') ;
	}

	if(isset($_POST['lc_swp_contact_string2'])) {
		update_post_meta($post_id, 'lc_swp_contact_string2', trim($_POST['lc_swp_contact_string2'])) ;
	} else {
		update_post_meta($post_id, 'lc_swp_contact_string2', '') ;
	}	
}
add_action('save_post', 'LUCILLE_SWP_contact_settings_meta_save');

function LUCILLE_SWP_artist_selection_save($post_id) {
	$is_autosave = wp_is_post_autosave( $post_id );
    $is_revision = wp_is_post_revision( $post_id );
...
add_action('save_post', 'LUCILLE_SWP_artist_selection_save');
